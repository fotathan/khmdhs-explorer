  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-slate-900">Authorities</h1>
        <p className="text-sm text-slate-500 mt-1">
          Directory of contracting authorities and awarding bodies
        </p>
      </motion.div>
        ...
        <form onSubmit={handleSearch} className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search authorities by name..."
      ...
      {loading ? (
        <LoadingSpinner text="Loading authorities..." />
      ) : (
        <div className="space-y-3">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="text-sm font-medium text-slate-800 truncate">{auth.name}</h3>
                  </div>
                  <div className="flex items-center gap-4 mt-2 text-xs text-slate-500">
                    {auth.city && (
                      <span className="flex items-center gap-1">
                        <MapPin className="w-3 h-3" /> {auth.city}
                      </span>
                    )}
                    {auth.vat_number && <span>VAT: {auth.vat_number}</span>}
                    {auth.nuts_code && <span>NUTS: {auth.nuts_code}</span>}
                  </div>
                </div>
                <div className="text-right flex-shrink-0 space-y-1">
                  ...
                  <p className="text-xs text-slate-400">{auth.act_count} acts</p>
                </div>
              </div>
            </motion.div>
          ))}
          ...
          {authorities.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-500">No authorities found</p>
            </div>
          )}
        </div>
      )}
