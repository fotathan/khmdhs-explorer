...
_SEARCH = {
    ...
    "Οι πίνακες στις σελίδες πράξης, αρχής και αναδόχου γίνονται κάρτες, μία ανά γραμμή, με μικρή ετικέτα πάνω από κάθε τιμή — και τα «Σύνολα ανά τύπο πράξης» μιας αρχής: ο τύπος πάνω, πράξεις, ακυρωμένες και αξία δίπλα, και ο σύνδεσμος «μόνο αυτές ›» πάντα ορατός. Μόνο ο πίνακας CPV στα Στατιστικά κρατά τις στήλες του και κυλά οριζόντια.":
        "Tables on the act, authority and contractor pages become cards, one per row, with a small label on each value — including an authority’s “Totals by act type”: the type on top, acts, cancelled and value beside it, and the “only these ›” link always visible. Only the CPV table in Analytics keeps its columns and scrolls sideways.",
...
_PARTY = {
    ...
    "‹ πίσω στον ανάδοχο": "‹ back to the contractor",
    "‹ πίσω στην αναθέτουσα": "‹ back to the authority",
    ...
    "Δεν υπάρχουν κατακυρωμένες συμβάσεις σε αυτούς τους κωδικούς CPV.": "No awarded contracts under these CPV codes.",
...
_ACTFORM = {
    ...
    # Multi-value authority / contractor party rows.
    "Αναθέτουσες αρχές": "Contracting authorities",
    "Ανάδοχοι / Νικητές": "Contractors / winners",
    "Προσθήκη αρχής": "Add authority",
    "Προσθήκη αναδόχου": "Add contractor",
    "Συνδεδεμένο": "Linked",
    "Ποσό ανάθεσης": "Award amount",
    "Αφαίρεση": "Remove",
    "Οδός": "Street",
    "Είδος": "Kind",
    "Τύπος αρχής": "Authority type",
    "Εξωτερικό ID": "External ID",
...
_HELP = {
    ...
    "Η εξαγωγή παίρνει τα φίλτρα όπως είναι τη στιγμή που πατάτε — και όσα αλλάξατε αφού άνοιξε η σελίδα — άρα το αρχείο περιέχει τα αποτελέσματα που βλέπετε στην οθόνη (έως το όριο). Το ίδιο ισχύει αν ανοίξετε τον σύνδεσμο εξαγωγής σε νέα καρτέλα ή τον αντιγράψετε.":
        "An export takes the filters as they are when you click — including any you changed after the page opened — so the file holds the results you see on screen (up to the cap). The same goes for opening the export link in a new tab or copying it.",
...
_DIGESTS = {
    ...
    "Η λίστα που ανοίγει εξηγεί και το «γιατί»: κάθε αποτέλεσμα δείχνει τη γραμμή «Ταιριάζει:» με τους όρους της αποθηκευμένης αναζήτησης που βρέθηκαν, και αν ανοίξετε μια πράξη από εκεί εμφανίζεται το πάνελ «Γιατί ταιριάζει» με τις επισημάνσεις μέσα στο κείμενο — ακριβώς όπως στην αναζήτηση. Οι όροι είναι αυτοί με τους οποίους στάλθηκε το email: μια μεταγενέστερη αλλαγή της αποθηκευμένης αναζήτησης δεν ξαναγράφει τα αποτελέσματα που έχουν ήδη σταλεί.":
        "The list it opens explains the “why” too: every result carries the “Matches:” line with the saved search's terms that were found, and opening an act from there shows the “Why it matches” panel with the highlights inside the text — exactly as in search. The terms are the ones the email was sent with: editing the saved search later does not re-label results that have already been mailed.",
...
_AI_POLICY = {
    ...
    'Η σύνοψη διαγωνισμού παράγεται με μοντέλο της DeepSeek. Η DeepSeek εδρεύει στη Λαϊκή Δημοκρατία της Κίνας και εκεί επεξεργάζει όσα της αποστέλλονται — το κείμενο της διακήρυξης διαβιβάζεται δηλαδή εκτός Ευρωπαϊκού Οικονομικού Χώρου. Αυτό που φεύγει είναι αποκλειστικά το ήδη δημοσιευμένο κείμενο του διαγωνισμού, το ίδιο έγγραφο που ο φορέας έχει αναρτήσει δημόσια, και ποτέ κάτι που αφορά εσάς, τον λογαριασμό σας ή την επιχείρησή σας.':
        'The tender summary is produced by a DeepSeek model. DeepSeek is based in the People\u2019s Republic of China and processes what is sent to it there \u2014 so the text of the tender documents is transferred outside the European Economic Area. What leaves is exclusively the already-published text of the tender, the same document the contracting authority has posted publicly, and never anything concerning you, your account or your business.',
    'Η πολιτική του παρόχου διέπει τη δική του πλευρά:':
        "The provider's own policy governs their side:",
    'όροι χρήσης':
        'terms of service',
    'Η σύνοψη διαγωνισμού παράγεται στο API της Anthropic (μοντέλα Claude). Σύμφωνα με τους εμπορικούς όρους του API, τα δεδομένα που αποστέλλονται και οι απαντήσεις δεν χρησιμοποιούνται για την εκπαίδευση μοντέλων· η πολιτική του παρόχου διέπει τη δική του πλευρά και είναι διαθέσιμη στο':
        "The tender summary is produced on the Anthropic API (Claude models). Under the commercial terms of that API, the data sent and the responses returned are not used to train models; the provider's own policy governs their side and is available at",
    'Οι υπόλοιπες λειτουργίες που καλούν μοντέλο — η αναγνώριση κειμένου σε σαρωμένα έγγραφα και, όπου είναι ενεργή, η σύνοψη κλήσεων — χρησιμοποιούν το API της Anthropic (μοντέλα Claude). Σύμφωνα με τους εμπορικούς όρους του API, τα δεδομένα που αποστέλλονται και οι απαντήσεις δεν χρησιμοποιούνται για την εκπαίδευση μοντέλων· οι όροι είναι διαθέσιμοι στο':
        'The remaining features that call a model \u2014 text recognition on scanned documents and, where it is switched on, the call summary \u2014 use the Anthropic API (Claude models). Under the commercial terms of that API, the data sent and the responses returned are not used to train models; the terms are available at',
...
_COMPANY_MATCH = {
    ...
    'Το πεδίο αναζήτησης ξεκινά συμπληρωμένο με την επωνυμία του προφίλ, οπότε η πρώτη αναζήτηση θέλει ένα κλικ. Μπορείτε να το αλλάξετε· μετά από κάθε αναζήτηση κρατά τον όρο που αναζητήθηκε.':
        'The search box starts filled in with the profile\'s company name, so the first search is one click. You can change it; after each search it keeps the term that was searched.',
    'Η σειρά κρίνεται κυρίως από την επωνυμία, συγκριμένη με τον όρο που γράψατε στο πεδίο — όχι με την επωνυμία του προφίλ. Μετράει πόσες από τις λέξεις σας περιέχει: για «food prime» η FOOD PRIME ΔΙΑΝΟΜΗ προηγείται της BT PRIME, που έχει μόνο τη μία λέξη. Η ύπαρξη στο μητρώο αναδόχων δίνει μικρό μπόνους που απλώς ξεχωρίζει ισοβαθμίες: δεν λέει ποια εταιρεία είναι ο πελάτης.':
        'The order is decided mainly by the name, compared with the term you typed in the box — not with the profile\'s company name. What counts is how many of your words it contains: for "food prime", FOOD PRIME ΔΙΑΝΟΜΗ ranks above BT PRIME, which has only one of the two words. Being in the contractor ledger adds a small bonus that only breaks ties: it says nothing about which company the customer is.',
