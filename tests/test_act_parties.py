...
# The directory rows this file inserts, by key.
_ORG_IDS = ["ORG-B1", "ORG-D1", "ORG-D2", "ORG-S1", "ORG-AF", "ORG-NM"]
_OPERATOR_VATS = ["099111222", "444555666"]


@pytest.fixture(autouse=True)
def _forget_parties(_clean):
    """_clean truncates the user side only. The acts the form saves (origin
    'authored', ΑΔΑΜ generated by the app) and the authorities/operators seeded
    for matching are removed afterwards, or later tests count them."""
    with connect() as c:
        before = [r["adam"] for r in c.execute(
            "SELECT adam FROM proc.procurement_act WHERE origin = 'authored'")]
    yield
    with connect() as c:
        c.execute("DELETE FROM proc.procurement_act "
                  "WHERE origin = 'authored' AND adam <> ALL(%s)", (before,))
        c.execute("DELETE FROM proc.authority WHERE org_id = ANY(%s)", (_ORG_IDS,))
        c.execute("DELETE FROM proc.economic_operator WHERE vat_number = ANY(%s)",
                  (_OPERATOR_VATS,))
